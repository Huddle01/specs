...
Media Nodes serve as the backbone of the dRTC Network, providing essential bandwidth and computational resources. Its responsibility is to establish WebRTC connections with the clients in the room . Media Nodes are responsible for processing media data, such as audio and video streams, and facilitating communication between clients. Media Nodes are incentivized to provide their services through the dRTC Network's token economy.
...
Media Layer: is where a Media Node processes media data within the dRTC Network. This layer is responsible for operations within WebRTC such as congestion control, employing algorithms like Simulcast to adapt video quality to the user's bandwidth, to scale based on the client's resource limitations. This Layer is an abstraction over the WebRTC SFU implementation used by the Media Node.
