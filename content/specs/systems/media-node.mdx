...
### Why a Worker?
While building the dRTC network, we wanted to build a network where users can monetize their resources by providing services to the network. The concept of Media Nodes self aligning themselves and doing leader election to provide resources to the network is a powerful concept and honestly seems like an ideal system, but 
becomes totally useless for larger scheme of things such as when thinking about `Cascading`, `QoS`, `Rate Limiting` etc, basically when we start asking questions of exactly how application building on RTC solves these complex problems. Every application trying to leverage the dRTC network will have different requirements and the a single set of protocol and standards can never fulfill all the requirements.
